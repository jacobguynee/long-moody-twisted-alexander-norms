#!/usr/bin/env python3
"""
hunt_counterexample_B4_v4.py

Almost-reducible pseudo-Anosov braids using the fast f^a * g^b approach.
Precompute f and g matrices, then binary-exponentiate.

Multiple families of (f, g) pairs designed to be "almost reducible":
the braid f^a * g^b should be pseudo-Anosov but close to having a
reducing curve.
"""
...
if __name__ == '__main__':
    ...
    families = [
        # (name, f_word, g_word)
        # Almost reducible: s1*s2 and s2^-1*s3
        ('f=s1.s2, g=s2^-1.s3',         [1, 2],     [-2, 3]),
        # Almost reducible: s1*s2 and s3
        ('f=s1.s2, g=s3',               [1, 2],     [3]),
        # Conjugated reducible
        ('f=s2.s1.s2^-1, g=s3',         [2, 1, -2], [3]),
        ('f=s2.s1.s2^-1, g=s2.s3.s2^-1',[2, 1, -2], [2, 3, -2]),
        # s1 and s2*s3*s2^-1 (this pair generates B4 but "almost" reduces)
        ('f=s1, g=s2.s3.s2^-1',         [1],        [2, 3, -2]),
        # Higher twist versions
        ('f=s1^2, g=s2.s3.s2^-1',       [1, 1],     [2, 3, -2]),
        ('f=s1, g=s2.s3^2.s2^-1',       [1],        [2, 3, 3, -2]),
        # Diagonal-like
        ('f=s1.s3, g=s2',               [1, 3],     [2]),
        ('f=s1.s3^-1, g=s2',            [1, -3],    [2]),
        ('f=s1.s3, g=s2^-1',            [1, 3],     [-2]),
        # Almost reducible with full twist flavor
        ('f=s1.s2.s1, g=s3',            [1, 2, 1],  [3]),
        ('f=s1.s2.s1, g=s3^-1',         [1, 2, 1],  [-3]),
        # Commutator-based
        ('f=[s1,s2], g=s3',             [1, 2, -1, -2], [3]),
        ('f=[s1,s2], g=s3^-1',          [1, 2, -1, -2], [-3]),
        ('f=s1, g=[s2,s3]',             [1],        [2, 3, -2, -3]),
        ('f=s1^-1, g=[s2,s3]',          [-1],       [2, 3, -2, -3]),
    ]
    ...
    for fam_name, f_word, g_word in families:
        print(f'\n=== {fam_name} ==='); sys.stdout.flush()

        # Precompute f and g in both representations
        fb = eval_word(f_word, bg, bgi)
        gb = eval_word(g_word, bg, bgi)
        fl = eval_word(f_word, lg, lgi)
        gl = eval_word(g_word, lg, lgi)

        for aa, bb in exponent_pairs:
